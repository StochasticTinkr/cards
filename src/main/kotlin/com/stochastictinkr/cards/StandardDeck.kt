    ...
    val cards = sequence {
        suits.forEach { suit ->
            indexes.forEach { index ->
                yield(DeckDefinition.Card(suit, index, suit.color, "${index.name} of ${suit.name}"))
            }
        }
        if (includeJokers) {
            val jokerIndex = DeckDefinition.Index("joker")
            colors.forEach { color ->
                yield(DeckDefinition.Card(suit = null, jokerIndex, color, "${color.name} ${jokerIndex.name}"))
            }
        }
    }.toList()
